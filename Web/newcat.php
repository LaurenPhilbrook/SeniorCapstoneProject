...
/* FORMAT TO ADD Subcategories TO A Category 

/category/{catid}/subcategory/{subcatid}


I think this means we have to first add the cat to get it an id and then fetch that id
the subcats values are their ids -- we will need to loop through for each element in the subcat array and add each one
with a put request 
...
*/
	...
	//GET CAT ID FROM DATABASE GIVEN NAME 
	...
	curl_setopt($curl2, CURLOPT_URL, "http://www.recycling-app-13.appspot.com/category/" . $catid . "/subcategory/" . $id);
	curl_exec($curl2);
	curl_close($curl2);
