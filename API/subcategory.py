	...
	def get(self, **kwargs):
		...
		if 'id' in kwargs:
			...
			for busId in businessIds:
				db.cursor.execute('SELECT id, name, phoneNumber, streetAddress, city, state, zipCode, hours, webpage, latitude, longitude FROM businesses_table WHERE id = ' + str (busId))
				for row in db.cursor.fetchall():
					businesses.append({'id':row[0], 'name':row[1], 'phoneNumber':row[2], 'streetAddress':row[3], 'city':row[4], 'state':row[5], 'zipCode':row[6], 'hours':row[7], 'webpage':row[8], 'latitude':row[9], 'longitude':row[10]})
